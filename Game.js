export default class MainGame extends Phaser.Scene
{
    ...
    selectEmoji (pointer, emoji)
    {
        ...
        if (emoji.frame.name == this.child1.frame.name)
        {
            //  Our first emoji
    ...
    newRound ()
    {
        ...
        this.tweens.add({
            targets: [this.emojisOne.getChildren(),this.describeText.text],
            scale: 0,
            ease: 'power2',
            duration: 600,
        ...
        // 0this.tweens.add({
        //     targets: this.describeText.text,
        //     scale: 0,
        //     ease: 'power2',
        //     duration: 600,
        //     delay: this.tweens.stagger(100, { grid: [ 4, 4 ], from: 'center' }),
    ...
    arrangeGrid ()
    {
        ...
        let frames = Phaser.Utils.Array.NumberArray(1, 38);
        let selected = Phaser.Utils.Array.NumberArray(0, 15);
        let children = this.emojisOne.getChildren();
        let remains = Phaser.Utils.Array.NumberArray(0, 15);
        //  Now we pick 16 random values, removing each one from the array so we can't pick it again
        //  and set those into the sprites
        let describeTexts = ["It is a smiling cat~~~", "It is a surprised cat!!!!!"]
        let selectedText = Phaser.Utils.Array.RemoveRandomElement(describeTexts);
        let selectEmoji = 0

        if(selectedText == "It is a smiling cat~~~")
        {
            selectEmoji = 39
            this.describeText.setText(selectedText)
        }
        else if(selectedText == "It is a surprised cat!!!!!")
        {
            selectEmoji = 40
            this.describeText.setText(selectedText)
        }
        ...
        this.child1.setFrame('smile' + selectEmoji);
        console.log('frame: ', selectEmoji, index1);
